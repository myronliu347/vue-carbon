...
<div class="page">
  ...
  <content>
    ...
    <content-block>
      <picker :slots="dateSlots" :visible-item-count="3" @picker-change="dateChange" :values="dates" ></picker>
      在校时间: <span style="color:#000;">{{dateStart.text}} - {{dateEnd.text}}</span>
    </content-block>
    <content-block>
...
export default {
  data () {
    ...
    return {
      years: [{values: years}],
      yearValues: years[10],
      dateSlots: [{values: dateSlots[0]}, {values: dateSlots[1]}],
      dates: [dateSlots[0][5], dateSlots[1][2]],
      dateStart: dateSlots[0][5],
      dateEnd: dateSlots[1][2],
      addressSlots: [{width: '100%', textAlign: 'right', values: Object.keys(address)}, {width: '100%', textAlign: 'left', values: ['北京']}],
      address: ['北京', '北京'],
      addressProvince: '北京',
      addressCity: '北京',
      date: '2014-12-23',
      time: '2014-12-23 22:23'
    }
  },
  methods: {
    back () {
    ...
    addressChange (values) {
      ...
      let arr = this.addressSlots[1]
      if (values[0] !== this.addressProvince) {
        arr = address[values[0]]
        this.addressSlots[1].values = arr
        this.addressSlots.$set(1, this.addressSlots[1])
        this.addressCity = arr[0]
      }
      this.addressProvince = values[0]
      this.$set('address', [this.addressProvince, this.addressCity])
    }
  }
}
</script>
